...
class Locale
{
    ...
    public function __construct()
    {
        if (!extension_loaded('intl')) {
            throw new \Zend\I18n\Exception\ExtensionNotLoadedException(sprintf(
	...
	protected $_localeData = array(
        'root'  => false, 'aa_DJ' => true, 'aa_ER' => true, 'aa_ET' => true, 'aa'    => true,
	...
	protected $_territoryData = array(
    ...
    public function getLangs($locale = false)
    {
    	if($locale === false){
    	...
    	$langs = array();
    	foreach($this->_localeData as $_locale => $_active){
			if(strpos($_locale, '_') === false && $_active){
				$langs[$_locale] = \Locale::getDisplayLanguage($_locale, $locale);
			}
		}
